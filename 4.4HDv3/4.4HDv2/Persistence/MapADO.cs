...
namespace robot_controller_api.Persistence
{
    public class MapADO : IMapDataAccess
    {
        ...
        public void UpdateMap(int id, Map updatedMap)
        {
            using var conn = new NpgsqlConnection(CONNECTION_STRING);
            conn.Open();
            using var cmd = new NpgsqlCommand("UPDATE map SET columns = @Columns, rows = @Rows, \"Name\" = @Name, modified_date = @ModifiedDate WHERE id = @Id", conn);
            cmd.Parameters.AddWithValue("@Id", id);
        ...
        public void AddMap(Map newMap)
        {
            ...
            using var cmd = new NpgsqlCommand("INSERT INTO map (columns, rows, \"Name\", description, created_date, modified_date) VALUES (@Columns, @Rows, @Name, @Description, @CreatedDate, @ModifiedDate)", conn);
