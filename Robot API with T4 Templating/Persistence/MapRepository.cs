    ...
    public class MapRepository : IMapDataAccess, IRepository
    {
        ...
        public void AddMap(Map newMap)
        {
            ...
            _repo.ExecuteReader<Map>(
                "INSERT INTO map (columns, rows, Name, description, created_date, modified_date) VALUES (@Columns, @Rows, @Name, @Description, @CreatedDate, @ModifiedDate);",
                sqlParams
            );
        }
        ...
        public void UpdateMap(int id, Map updatedMap)
        {
            ...
            _repo.ExecuteReader<Map>(
                "UPDATE map SET columns = @Columns, rows = @Rows, Name = @Name, description = @Description, modified_date = @ModifiedDate WHERE id = @Id;",
                sqlParams
            );
        }
